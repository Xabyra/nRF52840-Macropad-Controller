...
/* Number of consecutive identical reads required to consider a key stable */
#define DEBOUNCE_STABLE_SCANS 5
#define MATRIX_SETTLE_NOP_CYCLES 8
/* Divider correction factor from measured values: 3.938 V battery -> 2.089 V at ADC pin */
#define BATTERY_VOLTAGE_DIVIDER_NUM 1885
#define BATTERY_VOLTAGE_DIVIDER_DEN 1000
...
/* Стан кнопок */
/* key_state: debounced (stable) state exposed to the rest of the app */
static bool key_state[NUM_ROWS][NUM_COLS] = {false};
/* last_raw: last instantaneous raw read of the matrix line */
static bool key_last_raw[NUM_ROWS][NUM_COLS] = {false};
/* debounce counters: increment when raw == last_raw, reset when differs */
static uint8_t key_debounce_count[NUM_ROWS][NUM_COLS] = {0};
static bool enc_btn_state = false;
static bool enc_btn_last_raw = false;
static uint8_t enc_btn_debounce_count = 0;
...
static int battery_read_mv(void)
{
    ...
    return (mv * BATTERY_VOLTAGE_DIVIDER_NUM + BATTERY_VOLTAGE_DIVIDER_DEN / 2) / BATTERY_VOLTAGE_DIVIDER_DEN;
}
...
#define LOW_POWER_SYSTEM_OFF_TIMEOUT_MS (30U * 60U * 1000U)
...
{
    k_work_reschedule_for_queue(&k_sys_work_q, &inactivity_work,
                                K_MSEC(LOW_POWER_SYSTEM_OFF_TIMEOUT_MS));
#if DEBUG_INACTIVITY_LOG
    printk("Inactivity timer reset for system off: %u ms\n",
           LOW_POWER_SYSTEM_OFF_TIMEOUT_MS);
#endif
}
...
void input_enter_low_power_connected(void)
{
    ...
    backlight_was_on_before_sleep = backlight_on;
    force_backlight_off_for_sleep();
    printk("Power save: low-power connected mode entered\n");
}
...
void input_exit_low_power_connected(void)
{
    ...
    if (backlight_was_on_before_sleep) {
        set_backlight(current_brightness, true);
    }

    printk("Power save: low-power connected mode exited\n");
}
...
/* Global variable to store max random delay for sequences (set by Alt+F15 at first step) */
static uint16_t sequence_random_max_ms = 0;
...
#define HID_RETRY_DELAY_MS 10

static void ble_send_hid_reports(void);
static void hid_retry_work_handler(struct k_work *work);

static bool hid_retry_pending;
static K_WORK_DELAYABLE_DEFINE(hid_retry_work, hid_retry_work_handler);

static void hid_schedule_retry(void)
{
    hid_retry_pending = true;
    k_work_reschedule(&hid_retry_work, K_MSEC(HID_RETRY_DELAY_MS));
}

static int hid_notify_checked(const struct bt_gatt_attr *attr,
                              const void *data,
                              uint16_t len,
                              const char *tag,
                              bool retry_current_state)
{
    int err = bt_gatt_notify(NULL, attr, data, len);

    if (err == -ENOMEM) {
        printk("HID WARN: %s notify ENOMEM, scheduling retry\n", tag);
        if (retry_current_state) {
            hid_schedule_retry();
        }
        return err;
    }
    ...
    if (err) {
        printk("HID ERROR: %s notify failed err=%d\n", tag, err);
    }
...
static void ble_send_hid_reports(void) {
    ...
    hid_notify_checked(&hog_svc.attrs[6], kbd_report, sizeof(kbd_report),
                       "keyboard state", true);
    hid_notify_checked(&hog_svc.attrs[10], mouse_rep, sizeof(mouse_rep),
                       "mouse state", true);
}
...
static void input_send_encoder_mouse_wheel(Action_t act, bool clockwise, int steps)
{
    ...
    int err = hid_notify_checked(&hog_svc.attrs[10], report, sizeof(report),
                                 "encoder wheel", true);
    if (err) {
        return;
    }
    ...
    uint8_t release[4] = {0};
    hid_notify_checked(&hog_svc.attrs[10], release, sizeof(release),
                       "encoder wheel release", true);
#endif
}
...
static void input_send_encoder_key_pulse(Action_t act)
{
    ...
    int err = hid_notify_checked(&hog_svc.attrs[6], report, sizeof(report),
                                 "encoder key press", true);
    if (err) {
        return;
    }
    ...
    uint8_t release[8] = {0};
    hid_notify_checked(&hog_svc.attrs[6], release, sizeof(release),
                       "encoder key release", true);
}
...
static void sequence_thread_fn(void *p1, void *p2, void *p3) {
    ...
    uint8_t start_idx = 0;
    uint16_t max_random = 0;
    
    /* 1. ПЕРЕВІРКА ЗАГОЛОВКА РАНДОМІЗАЦІЇ (Alt + F15) */
    if (sq->len > 0) {
        SequenceStep_t *first = &sq->steps[0];
        
        /* Модифікатор Alt (0x04 або 0x40) */
        bool is_alt = (first->mod == 0x04 || first->mod == 0x40);
        
        /* Перевіряємо всі можливі варіанти коду F15 (децибел 72, хекс 0x48, 0x44 або стандартний HID 106/0x6A)
           щоб повністю синхронізуватися з Flutter */
        bool is_f15 = (first->code == 72 || first->code == 0x48 || first->code == 0x44 || first->code == 106 || first->code == 0x6A);
        
        if (is_alt && is_f15) {
            /* Перший крок — це конфігуратор. Беремо його delay_ms як максимальний рандом */
            max_random = first->delay_ms;
            start_idx = 1; /* Пропускаємо цей крок, не натискаємо його на ПК */
            printk("Sequence %d: Рандомізація увімкнена! Max random = %u ms (Alt+F15 пропущено)\n", idx, max_random);
        }
    }
    
    /* 2. ВИКОНАННЯ РЕАЛЬНИХ КРОКІВ ПОСЛІДОВНОСТІ */
    for (uint8_t i = start_idx; i < sq->len; i++) {
        SequenceStep_t *s = &sq->steps[i];
        
        uint16_t base_delay = s->delay_ms;
        uint16_t total_delay = base_delay;
        
        /* Якщо рандомізація активна, додаємо випадкове число від 0 до max_random */
        if (max_random > 0) {
            uint16_t random_offset = (uint16_t)(sys_rand32_get() % (max_random + 1));
            total_delay = base_delay + random_offset;
            printk("Sequence: Крок %u -> Базова: %u ms, Рандом: +%u ms, Чекаємо разом: %u ms\n", 
                   i, base_delay, random_offset, total_delay);
        } else {
            if (base_delay > 0) {
                printk("Sequence: Крок %u -> Чекаємо затримку: %u ms\n", i, total_delay);
            }
        }
        
        /* Спимо ПЕРЕД натисканням клавіші (чиста пре-затримка кроку) */
        if (total_delay > 0) {
            k_msleep(total_delay);
        }
        ...
        /* Use hid_notify_checked to handle -ENOMEM and schedule retries if needed */
        int rc;
        int attempts = 0;
        do {
            rc = hid_notify_checked(&hog_svc.attrs[6], report, sizeof(report), "sequence key press", false);
            if (rc == -ENOMEM) {
                k_msleep(HID_RETRY_DELAY_MS);
            }
            attempts++;
        } while (rc == -ENOMEM && attempts < 10);

        /* Стандартне утримання кнопки (10 мс), щоб ОС встигла зафіксувати клік */
        k_msleep(10);
        ...
        attempts = 0;
        do {
            rc = hid_notify_checked(&hog_svc.attrs[6], report_stop, sizeof(report_stop), "sequence key release", false);
            if (rc == -ENOMEM) {
                k_msleep(HID_RETRY_DELAY_MS);
            }
            attempts++;
...
static bool hardware_scan_matrix(void) {
    ...
    for (int c = 0; c < NUM_COLS; c++) {
        /* Drive one column active */
        gpio_pin_set_dt(&cols[c], 1);

        /* Small settling: a few NOP cycles to allow line to stabilise (very short, non-scheduler blocking)
         * This replaces k_busy_wait(30) to avoid long blocking delays. */
        for (volatile int i = 0; i < MATRIX_SETTLE_NOP_CYCLES; i++) {
            __asm__("nop");
        }

        for (int r = 0; r < NUM_ROWS; r++) {
            bool raw = (gpio_pin_get_dt(&rows[r]) == 1);

            /* If the raw reading is same as last raw, increment counter, else reset counter */
            if (raw == key_last_raw[r][c]) {
                if (key_debounce_count[r][c] < DEBOUNCE_STABLE_SCANS) {
                    key_debounce_count[r][c]++;
                }
            } else {
                key_debounce_count[r][c] = 1;
                key_last_raw[r][c] = raw;
            }

            /* If counter reached threshold and debounced state differs from exposed state, update */
            if (key_debounce_count[r][c] >= DEBOUNCE_STABLE_SCANS) {
                if (key_state[r][c] != key_last_raw[r][c]) {
                    key_state[r][c] = key_last_raw[r][c];
                    state_changed = true;
    ...
    /* Encoder button debouncing (polled) */
    if (device_is_ready(enc_btn.port)) {
        bool raw = (gpio_pin_get_dt(&enc_btn) > 0);

        if (raw == enc_btn_last_raw) {
            if (enc_btn_debounce_count < DEBOUNCE_STABLE_SCANS) enc_btn_debounce_count++;
        } else {
            enc_btn_debounce_count = 1;
            enc_btn_last_raw = raw;
        }

        if (enc_btn_debounce_count >= DEBOUNCE_STABLE_SCANS) {
            if (enc_btn_state != enc_btn_last_raw) {
                enc_btn_state = enc_btn_last_raw;
                state_changed = true;
                if (enc_btn_state) {
